...
&#091;layerswitch tiles="mapid1,mapid2,..." mapids="mapid3,mapid4,..." providers="provider1,provider2,..." opacity="mapid1,provider1,..."]
  ...
  <ul>';
	...
	$text = $text . '<li> ' . __( 'optional: <code>mapid</code> - This appears in the switching control.', 'extensions-leaflet-map' ) . '</li>';
	$text = $text . '</ul></li>';
	$text = $text . '
   <li> <code>&#091;layerswitch]</code><ul>';
	...
	$text = $text . '<li>' .
	__(
		'optional: <code>mapids</code> - a comma separated list with shortnames for providers. These appear in the switching control.',
		'extensions-leaflet-map'
	) . ' ' . sprintf(
