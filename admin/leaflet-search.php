...
&#091;leaflet-search propertyName="..." ...]</code></pre>';
	...
	$text = $text . '<p>' . sprintf(
		__( '%1$s must be before %2$s.', 'extensions-leaflet-map' ),
	...
	foreach ( $options as $option ) {
		if ( $option['default'] == '' && $option['param'] == 'marker' ) {
			$option['default'] = '(' . __( 'red circle', 'extensions-leaflet-map' ) . ')';
		}
		$new[] = array(
			'param'   => $option['param'],
			'desc'    => $option['desc'],
	...
	$text = $text . '<p>' . sprintf(
		__( 'See %1$sLeaflet Map Github page%2$s for more or less useful and possible options for %3$s.', 'extensions-leaflet-map' ),
...
&lt;/style></code></pre>';
	...
	$text = $text . '<p>' .
	sprintf(
		__( 'The specific style and css depends on your %1$stheme and taste%2$s', 'extensions-leaflet-map' ),
