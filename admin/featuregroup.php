...
function leafext_help_featuregroup() {
  ...
  ';
	...
	__('Not every option is appropriate for every element.','extensions-leaflet-map');

	$text=$text.' '.sprintf(__('Meaningful options may be %s.','extensions-leaflet-map'),
	'className, iconUrl, title, markerColor, className...  ');
...
<li style="margin-left: 1.5em;">'.
			sprintf(__('You can specify multiple %s and %s. If the %s are called the same, the elements will be placed in the same group despite having different options resp. properties. However, make sure that it is unique.','extensions-leaflet-map'),
	...
	sprintf(
		__('If %s contains %s and %s, then the elements for which the option / property specified in %s does not apply, are placed in the %s group. Elements whose property is not known are placed in the %s group. See also the developer console.','extensions-leaflet-map'),
		"<code>groups</code>",
		'"unknown"',
		'"others"',
...
[leaflet-featuregroup property="..." values="...,...,others,unknown" groups="...,...,Other elements,Unknown elements"]</code></pre>';
	...
	$othertext = '<h3>'.__('Notes','extensions-leaflet-map').'</h3>'.sprintf ( __('The parameter and settings for %s are valid too.','extensions-leaflet-map'),
