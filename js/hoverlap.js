...
function leafext_is_point_in_layer(map,point,layer) {
	...
	if (layer instanceof L.Circle) {
		if (debug) {
			console.log( "leafext_is_point_in_layer is L.Circle" );
		}
		...
		if (debug) {
			console.log( "leafext_is_point_in_layer is L.Polyline" );
		}
		...
		if (debug) {
			console.log( "leafext_is_point_in_layer is L.Polygon" );
		}
		...
		if (debug) {
			console.log( "leafext_is_point_in_layer is L.Path" );
		}
		let center   = layer.getLatLng();
		let radius   = layer.getRadius();
		var elements = [];
		...
		if (debug) {
			console.log( "leafext_is_point_in_layer is L.Marker" );
		}
	} else if ( layer.type == 'MultiPolygon' || layer.type == 'Polygon') {
		if (debug) {
			console.log( "leafext_is_point_in_layer is MultiPolygon or Polygon" );
		}
		...
		if (debug) {
			console.log( "leafext_is_point_in_layer is LineString" );
		}
		let polylatlngs = [];
		var length      = layer.coordinates.length;
		...
		if (debug) {
			console.log( "leafext_is_point_in_layer is Point" );
		}
		// console.log(layer);
	} else if ( layer.feature && layer.feature.geometry && layer.feature.geometry.type == 'GeometryCollection') {
		if (debug) {
			console.log( "leafext_is_point_in_layer is GeometryCollection" );
		}
		turf.meta.flattenEach(
			layer.toGeoJSON(),
			function (currentFeature, featureIndex, multiFeatureIndex) {
				if (debug) {
					console.log( "leafext_is_point_in_layer is in GeometryCollection", currentFeature.geometry.type );
				}
	...
	} else if ( layer.feature && layer.feature.geometry && layer.feature.geometry.type == 'MultiPoint') {
		if (debug) {
			console.log( "leafext_is_point_in_layer is MultiPoint" );
		}
	} else {
		if (debug) {
			console.log( "leafext_is_point_in_layer is other layer" );
		}
		if (debug) {
			console.log( layer );
		}
...
function leafext_marker_click(e,map,map_id,layer,all_options) {
	...
	if ( event == "click" ) {
		overlaplayers[map_id].eachLayer(
			function (layer) {
										...
										layer.eachLayer(
											function (geometry) {
												...
												if (geometry instanceof L.Path == true && geometry instanceof L.Polygon == false) {
													...
													geometry.off( 'click' );
												} else if (geometry instanceof L.Point || geometry instanceof L.Marker) {
