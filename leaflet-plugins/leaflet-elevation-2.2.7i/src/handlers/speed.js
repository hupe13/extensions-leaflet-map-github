export function Speed() {
	...
	return {
		...
		pointToAttr: (_, i) => {
			...
			return (dt && dt > 0 && dx > 0) ? Math.abs((dx / dt) * opts.timeFactor) * opts.speedFactor : '';
		},
		...
		tooltip: (this.options.speed) && {
			chart: (item) => L._('v: ') + parseFloat((item.speed).toPrecision(2)) + " " + speed.label,
			marker: (item) => parseFloat((item.speed).toPrecision(3)) + " " + speed.label,
			order: 50,
		},
		summary: (this.options.speed) && {
			"minspeed"  : {
				label: "Min Speed: ",
				value: (track, unit) => parseFloat((track.speed_min || 0).toPrecision(2)) + '&nbsp;' + unit,
				order: 51
			},
			"maxspeed"  : {
				label: "Max Speed: ",
				value: (track, unit) => parseFloat((track.speed_max || 0).toPrecision(2)) + '&nbsp;' + unit,
				order: 51
			},
			"avgspeed": {
				label: "Avg Speed: ",
				value: (track, unit) => parseFloat((track.speed_avg || 0).toPrecision(2)) + '&nbsp;' + unit,
				order: 52
			},
		}
	};
