export function Distance() {
	...
	return {
		...
		tooltip: opts.distance && {
			name: 'x',
			chart: (item) => L._("x: ") + (parseFloat((item[opts.xAttr]).toPrecision((item[opts.xAttr] > 10) ? 3 : 2)) || 0) + " " + distance.label,
			order: 20
		},
		summary: opts.distance && {
			"totlen"  : {
				label: "Total Length: ",
				value: (track) => (parseFloat((track.distance || 0).toPrecision((track.distance > 10) ? 3 : 2)) || 0) + '&nbsp;' + distance.label,
				order: 10
			}
		}
	};
